<template>
  <article class="p-3 rounded-10 border border-stroke bg-[#FAFAFA] text-sm">
    <div class="flex items-center gap-2 mb-3">
      <svg
        ...
        :class="props.data.side == 'buy' ? 'text-cgreen' : 'text-cred'"
      >
        <path
          fill="currentColor"
      ...
      <p>
        <span v-text="props.data.side == 'buy' ? 'خرید' : 'فروش'"></span>
        نام آیتم
      </p>
      <!-- {{ props.data.item }} -->
    </div>
    <p>
      شرح : خرید پولی فی آبشده (4/3318) 481،930،000 قطعی (نقد فردایی) به وزن
      {{ props.data.weightGram }} گرم به مبلغ
      {{ props.data.totalPrice.toLocaleString() }} ریال
    </p>
  </article>
</template>
